...
"""
This scripts preprocesses the data and prepares the fmm inputs.
When adding a new source to the framework it is required to add the data keys within this script
"""
...
def write_trip_file(df, cnt):
    ...
    elif SOURCE == "INCIDENT_BING":
        cord_arr = []
        Lat_From = df["incident_lat_from"]
        Long_From = df["incident_long_from"]
        Lat_To = df["incident_lat_to"]
        Long_To = df["incident_long_to"]
        for i in range(len(Lat_From)):
            cord_arr.append([[Long_From[i], Lat_From[i]], [Long_To[i], Lat_To[i]]])
        cords = cord_arr
