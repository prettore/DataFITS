from fmm import UBODTGenAlgorithm, Network, NetworkGraph, FastMapMatch, FastMapMatchConfig, GPSConfig, ResultConfig, \
...
def fmm(SOURCE, createUBODT, mode):
    global CITY
    network = Network(CITY + "_shp/edges.shp", "fid", "u", "v")  # Read the network from shape file
    ...
    if createUBODT:
        ...
        status = ubodt_gen.generate_ubodt(CITY + "_shp/ubodt.txt", 0.1, binary=False, use_omp=True)
        print(status)
