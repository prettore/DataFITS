...
def create_json_output(df,SOURCE):
    '''
    Hint: Big amounts of data leads to high RAM usage. Consider a smaller observation subset if the process should crash
    '''
    features = []
...
def create_csv_output(df, SOURCE):
    ...
    geojson_file_name = {
    "INCIDENT_BING" : "geojson_bingIncident.json",
    "INCIDENT_HERE" : "geojson_hereIncident.json",
    "TRAFFIC_HERE" : "geojson_hereTraffic.json",
    "CONSTRUCTION_OD" : "geojson_odConstruction.json",
    "TRAFFIC_OD" : "geojson_odTraffic.json",
    "Envirocar" : "geojson_envirocar.json",
    }
    ...
    for line in tqdm(f.readlines()):
        ...
        cpath_arr.append(list(set([int(cpath_val) for cpath_val in cpath.split(",")])))    # add the numeric values to a new array and remove double entries
    ...
    for entry in wrongEntry_IDs:
        ...
        if SOURCE == "TRAFFIC_OD": CNT_trafficOD -=1
        elif SOURCE == "TRAFFIC_HERE": CNT_trafficHERE -=1
        ...
        for index, row in df.iterrows():
            if SOURCE == "TRAFFIC_HERE":
                ...
                try:
                    ...
                    idx = traff_here_ids[(df["streetIntersection"].iloc[index],df["streetName"].iloc[index],df["streetDirection"].iloc[index])]
    ...
    #Rename Timestamp column (this is already done in preparation.py, but if the user did not prepare the data before its done here again)
    try:
