...
# Set timezone (required for UTC conversion)
local = pytz.timezone("Europe/Berlin")


def acquire_json(city, mode):
    """
    Implementation of OpenData acquisition
    """
    data = []
    ...
    if city == "bonn":
        ...
        if mode == "traffic":
            ...
            URL = "https://geoportal.stadt-koeln.de/arcgis/rest/services/verkehr/verkehrskalender/MapServer/0/query" \
                  "?where=objectid+is+not+null&text=&objectIds=&time=&geometry=&geometryType=esriGeometryEnvelope" \
                  ...
                  "=&historicMoment=&returnDistinctValues=false&resultOffset=&resultRecordCount=&returnExtentOnly" \
                  "=false&datumTransformation=&parameterValues=&rangeValues=&quantizationParameters=&featureEncoding" \
                  "=esriDefault&f=pjson "
    elif city == "Hamburg":
        if mode == "traffic":
            # GML file; extension to xml
            URL = "https://geodienste.hamburg.de/HH_WFS_Verkehrslage?SERVICE=WFS&VERSION=1.1.0&REQUEST=GetFeature" \
                  "&typename=de.hh.up:verkehrslage "
            # cords are given in epsg (epsg: 25832) format and have to be converted into lat long (epsg: 4326)
...
def traffic_to_csv(data_name, WRITE_MODE, CITY_NAME):
    ...
    if CITY_NAME == "bonn":
        ...
        if data_name == "traffic":
            ...
            try:
                df = pd.DataFrame({'Cords': coordinates[:], 'Road Id': road_id[:], 'Time': times[:], 'Speed': speed[:],
                                   'Traffic': traffic_status[:]})  # .sort_values(by=['Time'])
                # print(df)

                if not os.path.isfile("data_acquisition/data/bonn/Traffic_OD/od_bonn_" + str(data_name) + timestr + ".csv"):
                    HEADER_VAL = True
                    WRITE_MODE = "w+"
                else:
                    HEADER_VAL = False
                ...
                df.to_csv("data_acquisition/data/bonn/Traffic_OD/od_bonn_" + str(data_name) + timestr + ".csv", index=False,
                          encoding="utf-8", mode=WRITE_MODE, header=HEADER_VAL)
            ...
            try:
                df = pd.DataFrame({'Cords': coordinates[:], 'Construction Id': baustelle_id[:], 'Desc': bezeichnung[:],
                                   'Time': times[:], 'District': stadtbezirk[:], 'District Id': stadtbezirk_bez[:],
                                   'Adress': adresse[:], 'From': von[:], 'To': bis[:], 'Traeger': traeger[:],
                                   'Reason': massnahme[:], 'Blockage': sperrung[:]})  # .sort_values(by=['Time'])
                # print(df)

                if not os.path.isfile("data_acquisition/data/bonn/Construction_OD/od_bonn_" + str(data_name) + timestr + ".csv"):
                    HEADER_VAL = True
                    WRITE_MODE = "w+"
                else:
                    HEADER_VAL = False
                    WRITE_MODE = "a"
                df.to_csv("data_acquisition/data/bonn/Construction_OD/od_bonn_" + str(data_name) + timestr + ".csv", index=False,
                          encoding="utf-8", mode=WRITE_MODE, header=HEADER_VAL)
        ...
        if data_name == "traffic":
            ...
            try:
                ...
                return
            try:
                df = pd.DataFrame({'Cords': coordinates[:], 'ID': identifier[:], 'Time': times[:], 'Name': name[:],
                                   'Traffic': traffic_status[:], 'Link': link[:]})
                # print(df)

                if not os.path.isfile("data_acquisition/data/koeln/Traffic_OD/od_koeln_" + str(data_name) + timestr + ".csv"):
                    HEADER_VAL = True
                    WRITE_MODE = "w+"
                else:
                    HEADER_VAL = False
                    WRITE_MODE = "a"
                df.to_csv("data_acquisition/data/koeln/Traffic_OD/od_koeln_" + str(data_name) + timestr + ".csv", index=False,
                          encoding="utf-8", mode=WRITE_MODE, header=HEADER_VAL)
            except Exception as x:
                logging.error("Error acquiring traffic open data for " + str(CITY_NAME))
                logging.error(x)
            ...
            try:
                df = pd.DataFrame(
                    {'Cords': coordinates[:], 'Construction Id': object_id[:], 'Time': times[:], 'Adress': adresse[:],
                     'From': von[:], 'To': bis[:], 'Typ': typ[:]})  # .sort_values(by=['Time'])
                # print(df)
                ...
                if not os.path.isfile("./koeln/Construction_OD/od_koeln_" + str(data_name) + timestr + ".csv"):
                    ...
                    HEADER_VAL = False
                    WRITE_MODE = "a"
                df.to_csv("data_acquisition/data/koeln/Construction_OD/od_koeln_" + str(data_name) + timestr + ".csv", index=False,
                          encoding="utf-8", mode=WRITE_MODE, header=HEADER_VAL)
                # print("### Data acquired and saved ###")
            except Exception as x:
                logging.error("Error acquiring construction open data for " + str(CITY_NAME))
...
def main(searchCity, mode, time, logging_lvl):
    ...
    for city in cities:
        if not os.path.isdir('data_acquisition/data/' + city + '/'):
            os.makedirs('data_acquisition/data/' + city + '/')  # Creates the od folders to save the data
        if not os.path.isdir('data_acquisition/data/' + city + '/Construction_OD/'):
            os.makedirs('data_acquisition/data/' + city + '/Construction_OD')
        if not os.path.isdir('data_acquisition/data/' + city + '/Traffic_OD/'):
            os.makedirs('data_acquisition/data/' + city + '/Traffic_OD')
